#!/usr/bin/env python3
"""
NEB calculation of H vacancy-hopping in pyrite FeS2
using MACE-MP-0 large model on GPU.

Crystal: Pa3 (205), a=5.418 A, 2x2x2 supercell (96 atoms)
Special: inter-dimer S-S pair selection (d > 2.5 A, skip S2^2- dimers)
Output: q071_neb_pyrite_large.json, q071_neb_pyrite_large.png
"""
...
def main():
    t_total = time.time()
    results = {"mineral": "pyrite", "model": "MACE-MP-0 large", "device": "cuda"}
    ...
    # ── [2/8] Build pyrite structure (2x2x2 supercell) ───────────────
    print("[2/8] Build pyrite 2x2x2 supercell")
    t0 = time.time()
    unit = crystal(
        symbols=["Fe", "S"],
        basis=[(0, 0, 0), (0.386, 0.386, 0.386)],
        spacegroup=205,
    ...
    # ── [5/8] Find inter-dimer S-S pair ──────────────────────────────
    print("[5/8] Find inter-dimer S-S pair (PBC, d > 2.5 A)")
    t0 = time.time()
    ...
    print(f"  Found {len(dimer_pairs)} S2 dimers (d < 2.5 A)")

    # Find nearest inter-dimer pair (d > 2.5 A, preferably < 4 A)
    min_d = np.inf
    si, sj = -1, -1
    for a in range(len(s_indices)):
        for b in range(a + 1, len(s_indices)):
            d = d_matrix[a, b]
            if d > 2.5 and d < 4.0 and d < min_d:
                min_d = d
                si, sj = a, b

    # Fallback: if no pair in (2.5, 4.0), widen range
    if si == -1:
        print("  No pair in (2.5, 4.0) A range, widening search...")
        for a in range(len(s_indices)):
            for b in range(a + 1, len(s_indices)):
                d = d_matrix[a, b]
                if d > 2.5 and d < min_d:
                    min_d = d
                    si, sj = a, b

    si_idx, sj_idx = s_indices[si], s_indices[sj]
    print(f"  Inter-dimer S pair: atoms {si_idx} & {sj_idx}, d = {min_d:.3f} A")
    results["S_pair_indices"] = [int(si_idx), int(sj_idx)]
    results["S_pair_distance_A"] = float(min_d)
    results["n_dimers"] = len(dimer_pairs)
    print(f"  done in {time.time()-t0:.1f}s")
    ...
    # Save JSON
    json_path = RESULTS / "q071_neb_pyrite_large.json"
    ...
    if neb_energies is not None:
        ...
        ax.plot(x, neb_energies_rel, "m^-", linewidth=2, markersize=8)
        ax.set_xlabel("Reaction coordinate", fontsize=13)
        ax.set_ylabel("Energy (eV)", fontsize=13)
        ax.set_title(
            f"Pyrite H vacancy-hopping NEB (2x2x2 supercell)\n"
        ...
        text_lines = [
            ...
            f"  Note: inter-dimer hop (d > 2.5 A)",
        ]
        ax.text(
            0.02, 0.98, "\n".join(text_lines),
        ...
        fig.tight_layout()
        png_path = RESULTS / "q071_neb_pyrite_large.png"
