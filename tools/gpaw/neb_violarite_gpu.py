#!/usr/bin/env python3
"""
NEB calculation of H vacancy-hopping in violarite FeNi2S4
using MACE-MP-0 large model on GPU.

Crystal: Fd3m (227), a=9.464 A
Output: q071_neb_violarite_large.json, q071_neb_violarite_large.png
"""
...
def main():
    t_total = time.time()
    results = {"mineral": "violarite", "model": "MACE-MP-0 large", "device": "cuda"}
    ...
    # ── [2/8] Build violarite structure ──────────────────────────────
    print("[2/8] Build violarite structure")
    t0 = time.time()
    atoms = crystal(
        symbols=["Fe", "Ni", "S"],
        basis=[(0, 0, 0), (5/8, 5/8, 5/8), (0.261, 0.261, 0.261)],
        spacegroup=227,
    ...
    # Save JSON
    json_path = RESULTS / "q071_neb_violarite_large.json"
    ...
    if neb_energies is not None:
        ...
        ax.plot(x, neb_energies_rel, "gs-", linewidth=2, markersize=8)
        ax.set_xlabel("Reaction coordinate", fontsize=13)
        ax.set_ylabel("Energy (eV)", fontsize=13)
        ax.set_title(
            f"Violarite H vacancy-hopping NEB\n"
        ...
        fig.tight_layout()
        png_path = RESULTS / "q071_neb_violarite_large.png"
