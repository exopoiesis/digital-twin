#!/bin/bash
# vast_dashboard_inner.sh — версия для запуска ВНУТРИ контейнера skypilot
# Вызывается из dashboard.bat: docker exec skypilot bash /tmp/vast_dashboard_inner.sh
...
# Get instance list (direct call, no docker exec)
INSTANCES=$(vastai show instances 2>/dev/null)
if [ -z "$INSTANCES" ]; then
...
# Header
echo -e "  ${B}STATUS  LABEL          VRAM/RAM      vCPU  \$/hr  RUN      KILL   PROGR.       TASK${N}"
echo    "  ------  -------------- ------------- ----- -----  -------  -----  -----------  ------------------------------------------"
...
LK=$(grep -rh "Exit code\|signal [0-9]\|Killed" /workspace/*watchdog*.log /workspace/results/*watchdog*.log /workspace/results/*/watchdog*.log /workspace/results/*_stderr.log 2>/dev/null|tail -1|grep -oP "\d{2}:\d{2}"|tail -1);
[ -z "$LK" ]&&LK="-";
printf "%s|%s|%s/%s|%s/%s|%s|%s|%s|%s|%s" "$ST" "$CPU" "$VU" "$VT" "$RU" "$RT" "$XC" "$SC" "$ET" "$LK" "$TASK"
'

# Read from fd3, leaving stdin free for ssh
while IFS='|' read -r id ssh_addr ssh_port label gpu vcpu ram cost <&3; do
  DATA=$(ssh $SSH_OPTS -p "$ssh_port" "root@$ssh_addr" "$PROBE" 2>/dev/null </dev/null)
  ...
  IFS='|' read -r status cpu vram_info ram_info xyz_n scf_n elapsed last_kill task <<< "$DATA"
  ...
  # Format process elapsed time: seconds -> XdYhZm
  et=${elapsed:-0}
  et_d=$((et / 86400))
  et_h=$(( (et % 86400) / 3600 ))
  et_m=$(( (et % 3600) / 60 ))
  if [ "$et" -eq 0 ]; then
    ...
    runtime="${et_d}d${et_h}h${et_m}m"
  elif [ "$et_h" -gt 0 ]; then
    runtime="${et_h}h${et_m}m"
  else
    runtime="${et_m}m"
  fi

  # One-line output: status first, then fixed-width fields
  rest=$(printf "%-14s %-13s %5s  \$%-5s %-8s %-6s %-12s %s" \
    "$label" "${vram_info}|${ram_info}" "$vcpu" "$price" "$runtime" "${last_kill:-'-'}" "$prog" "$task")
  echo -e "  ${sc}  ${rest}"
