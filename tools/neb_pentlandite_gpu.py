...
Output: q071_neb_pentlandite_large.json, q071_neb_pentlandite_large.png
"""
...
def main():
    t_total = time.time()
    results = {"mineral": "pentlandite", "model": "MACE-MP-0 large", "device": "cuda"}
    ...
    print("[2/8] Build pentlandite structure")
    t0 = time.time()
    atoms = crystal(
        symbols=["Ni", "Fe", "S"],
    ...
    # Recalculate sj_idx after deletion: if sj_idx > si_idx, shift down by 1
    sj_idx_shifted = sj_idx - 1 if sj_idx > si_idx else sj_idx
    config2 = atoms.copy()
    del config2[[sj_idx]]
    ...
    try:
        ...
        # Endpoint A: H near site si
        endA = atoms.copy()
        # Delete both S atoms (delete higher index first to preserve lower)
        ...
        # Create images (endpoints + intermediates all need a calculator)
        start = endA.copy()
        start.calc = calc
        images = [start]
        for _ in range(11):
    ...
    # Save JSON
    json_path = RESULTS / "q071_neb_pentlandite_large.json"
    ...
    if neb_energies is not None:
        ...
        ax.set_title(
            f"Pentlandite H vacancy-hopping NEB\n"
        ...
        y_min = min(neb_energies_rel)
        y_max = max(neb_energies_rel)
        text_lines = [
            "Comparison:",
        ...
        fig.tight_layout()
        png_path = RESULTS / "q071_neb_pentlandite_large.png"
